...
## Project Overview
L1NE is a Zig monorepo structured like a Rust workspace. Key areas: `src/l1ne/` holds the orchestrator, `src/parsers/nix-zig/` the Nix parser, `src/parsers/ccl-zig/` the CCL parser, and `src/l1ne/simulator/dumb-server/` a Rust demo. Artifacts generated by the Zig build land in `zig-out/`; treat it as throwaway state.
...
## No Gambiarra Policy
Follow the zero-hack rule from `ai/CLAUDE.md`:
...
Use `src/l1ne/simulator/config/` as canonical reference—copy locally before experimentation. Extend `src/l1ne/simulator/scenarios/` when introducing new orchestration behaviour. Clean `zig-out/` or `.zig-cache/` before PRs to avoid noisy diffs.
